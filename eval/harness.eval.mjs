...
describe('harness eval', { skip: !(await lmStudioAvailable()) && 'LM Studio not available' }, () => {
  let tmpDir;
  ...
  it('creates a file when asked', { timeout: 120_000 }, async () => {
    ...
    assert.ok(result.filesChanged.length > 0, 'should change at least one file');
  ...
  it('reads and modifies an existing file', { timeout: 120_000 }, async () => {
    ...
    assert.ok(content.includes('+') || content.includes('a + b'), 'should fix the subtraction to addition');
  });
  ...
  it('uses tools to explore before writing', { timeout: 120_000 }, async () => {
    ...
    await writeFile(join(tmpDir, 'explore-test/app.mjs'), 'import config from "./config.json" assert { type: "json" };\n');
