...
describe('write_file', () => {
  ...
  it('writes a new file', async () => {
    const result = await writeFileTool.execute({ path: 'out.txt', content: 'data' }, context);
  ...
  it('creates parent directories', async () => {
    const result = await writeFileTool.execute({ path: 'a/b/c.txt', content: 'nested' }, context);
    assert.equal(result.written, true);
  ...
  it('rejects paths escaping workspace', async () => {
    const result = await writeFileTool.execute({ path: '../../evil.txt', content: 'bad' }, context);
    assert.ok(result.error);
  });
...
describe('search', () => {
  ...
  it('finds matches in text files', async () => {
    await writeFile(join(tmpDir, 'code.mjs'), 'function hello() {\n  return "hi";\n}\n');
  ...
  it('respects glob filter', async () => {
    ...
    const result = await searchTool.execute({ pattern: 'target', glob: '.mjs' }, context);
    assert.equal(result.matches.length, 1);
...
describe('run_command', () => {
  ...
  it('captures non-zero exit codes', async () => {
    const result = await runCommandTool.execute({ command: 'exit 42' }, context);
